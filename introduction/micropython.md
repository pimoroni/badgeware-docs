...
The good news is that it's *genuinely* Python. The syntax, control flow, functions, classes, list comprehensions, f-strings, exceptions — all there, all familiar. If you know Python, you already know MicroPython. But because it's running on a small, self-contained computer rather than a laptop, there are a handful of differences worth knowing before they catch you out.

# Key differences

MicroPython is a version of Python, but it's been trimmed and tuned to run well on resource-constrained devices. That leads to a handful of differences worth keeping in the back of your mind.

### Fewer packages

The everyday modules (`time`, `math`, `random`, `json`) are here, but plenty of packages you might be used to aren't — heavyweight things like video processing or advanced maths just don't fit on a microcontroller. MicroPython has its own package manager, [mip](https://docs.micropython.org/en/latest/reference/packages.html), for grabbing more, though for badge apps you'll mostly lean on Badgeware's APIs anyway.

### Memory is finite

The badge gives MicroPython an **8MB heap** — great for a microcontroller, and plenty for most apps. But there's no swap to fall back on, so you have to stay within it: very large data or a slow leak will eventually trip a `MemoryError`.

### Lower precision numbers

Whole numbers are arbitrary-precision, just like Python. Floats, however, are usually **single-precision** rather than 64-bit, so expect a touch less decimal accuracy — fine for drawing and everyday maths. Speed isn't a concern, mind: the RP2350 crunches floats in hardware, and either way MicroPython's own overhead far outweighs any time spent waiting on the maths.
...
MicroPython more or less *is* the system, so full `threading`, `asyncio` and `multiprocessing` aren't really a thing. You don't need them on the badge — the draw-and-`update()` loop is how you stay responsive.
...
Interpreted Python on a microcontroller is much slower than on your desktop. For static or lightly interactive apps that's rarely a problem — but anything animation-heavy, and games in particular, can push the badge hard, and hitting a smooth frame rate there often takes real care.

A few techniques go a long way when you need the speed:
...
- **Allocate as little as possible.** Creating objects inside a tight loop triggers garbage collection, which causes pauses. Preallocate buffers once and reuse them, favour `bytearray` and in-place operations, and avoid building throwaway objects each frame.
