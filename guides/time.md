---
title: Time
summary: Measure elapsed time for animations and timing, and keep the real date and time using the badge's clocks, alarms and NTP.
icon: timer
---
...
- **How much time has passed?** - for animating things, timing events, or keeping movement smooth. This is handled by the badge's *system clocks*, which count up from when it was powered on.
...
`badge.ticks`\
The number of milliseconds since the badge was powered on, as of when the current frame started. Handy as a steadily increasing value to drive animations and blinking.
...
The badge won't always render at exactly the same rate, so if you move something by a fixed amount every frame it'll speed up and slow down as the framerate changes. Instead, multiply by `ticks_delta` to move by an amount of time rather than an amount of frames:
...
However fast or slow the badge is drawing, the object now moves at a steady 60 pixels per second.
...
The badge has **two** real-time clocks working together:

- **The RP2350's internal clock.** This is part of the main chip and keeps the date and time while the badge is powered. It's what MicroPython's `time` module reads from, but it loses track the moment power is removed.
- **The onboard RTC.** This is a separate low-power chip (a PCF85063A) with its own backup power, so it keeps time through sleep and even when the badge is switched off, as long as there's charge left in the battery. It can also wake the badge with alarms and timers.

The two are kept in sync automatically. When the badge starts up, it copies the time from whichever clock has a sensible value into the other, so the battery-backed onboard RTC effectively restores the correct time to the main chip after the badge has been off.
...
The onboard RTC can raise an alarm at a future time, which is useful for waking the badge from sleep or triggering something at a set moment.
...
If the badge is connected to Wi-Fi, it can fetch the correct time from an internet time server (NTP) rather than having you set it by hand.

`rtc.time_from_ntp()`\
Fetches the current time over the network and writes it to both clocks. Requires a working Wi-Fi connection (see the networking guide for connecting).
